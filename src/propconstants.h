...
{
    PROPFLAG_ALLOWFALL                         = 0x00000001, // Allows this object to fall when its support is destroyed or destruction triggers falling. Overridden by No fall.
    PROPFLAG_ONSIDE                            = 0x00000002, // Sideways placement, anchored at the model's Z-min face.
    ...
    PROPFLAG_FORCE_COLLISIONS                  = 0x00000100, // Blocks player and character movement.
    ...
    PROPFLAG_ILLUMINATED                       = 0x00000400, // Ignore stan color.
    PROPFLAG_FREE_STANDING_GLASS               = 0x00000800, // Glass not embedded in another object.
    PROPFLAG_ABSOLUTEPOSITION                  = 0x00001000, // Preserve the object's position when not on a stan tile instead of snapping to its pad.
    PROPFLAG_AIUNDROPPABLE                     = 0x00002000, // Prevents automatic dropping when the carrying character dies or drops concealed items. Scripts can still explicitly transfer or drop the item.
    PROPFLAG_ASSIGNEDTOCHR                     = 0x00004000, // Gives this object to a character when the level loads. The Pad field specifies the character ID instead of a placement pad.
    PROPFLAG_INSIDEANOTHEROBJ                  = 0x00008000, // Embedded object
    PROPFLAG_FORCEMORTAL                       = 0x00010000, // Allows normally protected collectibles, such as keys, hats, and ammo crates, to take damage and be destroyed.
    PROPFLAG_INVINCIBLE                        = 0x00020000, // Invincible
    PROPFLAG_ALLOW_PICKUP                      = 0x00040000, // Allows players to collect this normally noncollectible object by approaching it. Standard weapons, ammo, keys, and armor do not require this flag.
    ...
    PROPFLAG_EMBEDDED                          = 0x01000000, // Prevents noncollectible objects from taking damage when they land or bounce. Does not place the object inside another object.
    ...
    PROPFLAG_AUTOGUN_DISABLED                  = 0x10000000, // Disables turret aiming and firing. The turret remains visible. Also set automatically when destroyed.
