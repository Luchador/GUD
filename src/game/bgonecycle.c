...
static void bgOneCycleResetState(BgOneCycleState *state)
{
    ...
    state->fogPolicy = BG_FOG_AUTO;
    state->environmentPolicy = BG_ENV_AUTO;
...
static s32 bgOneCycleReadState(BgOneCycleState *state, Gfx command, bool cutouts)
{
    ...
    } else if (BG_ENV_IS_MARKER(command.words.w0, command.words.w1)) {
        if ((command.words.w1 & 255u) <= BG_ENV_LINEAR)
            state->environmentPolicy = command.words.w1 & 255u;
    } else if (BG_FOG_IS_MARKER(command.words.w0, command.words.w1)) {
        if ((command.words.w1 & 255u) <= BG_FOG_OFF)
            state->fogPolicy = command.words.w1 & 255u;
        ...
        bool model, bool cutouts, bool character)
{
    ...
    *chosen = *source;
    ...
    if (source->fogPolicy != BG_FOG_AUTO || source->environmentPolicy != BG_ENV_AUTO) return FALSE;
        ...
        const Gfx *initial, s32 initialSize, bool cutouts, bool character)
{
    ...
    for (i = 0; i < size / sizeof(Gfx); i++) {
        ...
        if (opcode == (u8)G_DL) {
            ...
            fogPolicy = source.fogPolicy;
            environmentPolicy = source.environmentPolicy;
            bgOneCycleResetState(&source);
            source.fogPolicy = fogPolicy;
            source.environmentPolicy = environmentPolicy;
