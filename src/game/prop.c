...
#ifdef NONMATCHING
s32 load_proptype(s32 type)
{
    s32                  count   = 0;
    PropDefHeaderRecord *propdef = g_CurrentSetup.propDefs[0];

    if (propdef != NULL)
    {
        if (propdef->type != PROPDEF_END)
        {
            for (; propdef->type != PROPDEF_END; propdef += sizepropdef(propdef->type))
            {
                if ((type & 0xFF) == propdef->type)
                {
                    count++;
                }
            }
...
/* 036688 7F001B58 3C108007 */  lui   $s0, %hi(g_CurrentSetup+0x0c)
/* 03668C 7F001B5C 8E105D0C */  lw    $s0, %lo(g_CurrentSetup+0x0c)($s0)
