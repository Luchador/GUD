...
void objHit(ShotData *shotdata, BulletHit *hit)
{
    ...
    {
        fxCreateBulletSpark(&pos, SPARK_LASER, rootprop->stan->room);
    }
    else if (shotdata->weapon == ITEM_WATCHLASER)
    {
        fxCreateBulletSpark(&pos, SPARK_WATCHLASER, rootprop->stan->room);
    }
    else
    {
        fxCreateBulletSpark(&pos, SPARK_STANDARD, rootprop->stan->room);
    }
...
void door7F05522C(DoorRecord *door, f32 *arg1, f32 *arg2, s32 altcoordsystem)
{
    ...
    if (1) { scale = 1.0f; }
    playerpos.f[0] = (((g_CurrentPlayer->spatialState.facingDirection.x * 30.0f) * scale) * 0.75f) + playerprop->pos.x;
    playerpos.f[1] = playerprop->pos.y;
    playerpos.f[2] = (((g_CurrentPlayer->spatialState.facingDirection.z * 30.0f) * scale) * 0.75f) + playerprop->pos.z;
