...
s32 g_CurrentStageToLoad = LEVELID_DEFAULT;
...
/* Fixed-size slots let GEditor replace budgets without moving linked data.
 * Both arrays derive their slots from the same catalog, so reordering a row
 * carries its allocation string with it. No gameplay code uses these slots. */
#define LEVEL(id, name, setup, bg, stan, memory, scale, render, lod, music, sound, extra) memory,
...
#define LEVEL(id, name, setup, bg, stan, memory, scale, render, lod, music, sound, extra) \
    {id, name, setup, bg, stan, g_LevelMemoryAllocationStrings[LEVEL_SLOT_##id], \
     scale, render, lod, music, sound, extra},
struct LevelEntry g_LevelInfoTable[LEVEL_INFO_COUNT] = {
#define LEVEL_PLACEHOLDER(id, name, setup, bg, stan, memory, scale, render, lod, music, sound, extra) \
    {id, name, setup, bg, stan, NULL, scale, render, lod, music, sound, extra},
#include "leveltable.inc"
#undef LEVEL_PLACEHOLDER
};
#undef LEVEL
