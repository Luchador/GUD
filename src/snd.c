...
void sfx_c_70007E80(ALSndPlayer *sndp, ALSndpEvent *event)
{
    ...
    ALLink linky;                       // sp 0x74; linky.next: 116, linky.prev: 120
    ...
    allocVoiceSuccess = 0;
    linky.next = NULL;
    // typo/mistake, linky.prev is unitialized.
    ...
    while (1)
    {
        ...
        switch (event->unsignedType)
        {
            case (AL_SNDP_PLAY_EVT):
            {
                if ((state->unk3f == AL_UNKOWN_5) || (state->unk3f == AL_UNKOWN_4))
                {
                    ...
                    vc.fxBus = 0;
                    vc.priority = (s16)state->priority;
                    vc.unityPitch = 0;
                    voice = &state->voice;
                        ...
                        if (compare_result)
                        {
                            ...
                            do
                            {
                                    ...
                                    && (tstate->unk3f != AL_UNKOWN_3))
                                {
                                    ...
                                    voice = &tstate->voice;
                                    alSynSetVol(sndp->drvr, (ALVoice*)voice, 0, DELTA_1_MS);
                ...
                )
                {
                    switch (state->unk3f)
                    {
                        case (AL_PLAYING):
                        {
                            ...
                            alSynSetVol(sndp->drvr, (ALVoice *)&state->voice.table, 0, delta);
            ...
            case (AL_SNDP_VOL_EVT):
            {
                ...
                if (state->unk3f == AL_PLAYING)
                {
                    ...
                    alSynSetVol(sndp->drvr, (ALVoice *)&state->voice.table, (s16) vtmp, DELTA_1_MS);
                }
            }
            break;
            ...
            case (AL_SNDP_UNKNOWN_09_EVT):
            {
                ...
                {
                    play_sfx_a1((struct ALBankAlt_s *)event->unk_u_1.unkC, event->unk_u_1.unkA, state->state);
                }
            }
            break;
        ...
        state = (ALSoundState *)linky.next;

        if (linky.next == NULL || msgtype != 0)
        {
            // do nothing
        }
        else
...
ALSoundState *play_sfx_a1(struct ALBankAlt_s *arg0, s16 arg1, ALSoundState *arg2)
{
    ...
    do
    {
        ...
        if (state != NULL)
        {
            ...
            delta_temp_s3 = sound->keyMap->velocityMax * DELTA_33_MS;

            if ((state->unk3e & 0x10) != 0)
            {
                state->unk3e &= (~(s16)(0x10));
