                 ...
                 double renderscale, int start, int end)
{
    ...
    if (!isfinite(nearclip) || !isfinite(farclip) || !isfinite(renderscale) || nearclip <= 0 || farclip <= nearclip || renderscale <= 0 || range <= 0)
    { 
        return 0; 
    }
    ...
    if (multiplier <= 0 || multiplier > INT16_MAX || offset < INT16_MIN || offset > INT16_MAX) 
    { 
        return 0; 
    }
    ...
    if (!isfinite(result.constant) || !isfinite(result.reciprocal) || result.constant <= 0 || result.reciprocal <= 0 || !isfinite(worldfar) || worldfar > FLT_MAX) 
    { 
        return 0; 
    }
    ...
    if (!isfinite(linearstart) || !isfinite(linearend) || linearstart < 0 || linearend > FLT_MAX || linearend <= linearstart)
    { 
        return 0; 
    }
    ...
    if (result.linearend <= result.linearstart) 
    { 
        return 0; 
    }
