...
typedef struct IssuesWindow { LevelIssueReport report; BOOL stale; } IssuesWindow;
...
{
    IssuesScanRequest request = {0}; char summary[160];
    state->stale = TRUE; Clear(dialog, state);
    SetDlgItemText(dialog, IDC_ISSUES_STATUS, "Checking the current level...");
    ...
    if (!ok)
    {
        ...
        SetDlgItemText(dialog, IDC_ISSUES_STATUS, "The check could not be completed. Choose Check Again to retry.");
    ...
    for (DWORD i = 0; i < state->report.count; i++)
    {
        ...
        ListView_SetItemText(list, i, 1, issue->subject);
        ListView_SetItemText(list, i, 2, issue->description);
    }
    SendMessage(list, WM_SETREDRAW, TRUE, 0); InvalidateRect(list, NULL, TRUE);
    if (state->report.count)
    {
        snprintf(summary, sizeof(summary), "%lu errors, %lu warnings.%s Checked current level data, including unsaved edits.",
    ...
    else { strcpy(summary, "No issues found by these checks. Checked current level data, including unsaved edits."); }
...
static void Layout(HWND dialog)
{
    ...
    int details = bottom - gap - height * 3, top = gap + height * 2;
    MoveWindow(GetDlgItem(dialog, IDC_ISSUES_STATUS), margin, gap, width - margin * 2, height * 2 - gap, TRUE);
    ...
    ListView_SetColumnWidth(GetDlgItem(dialog, IDC_ISSUES_LIST), 2, width - margin * 2 - button * 3 - 26);
}
static INT_PTR CALLBACK Dialog(HWND dialog, UINT message, WPARAM wp, LPARAM lp)
{
    ...
    if (message == WM_INITDIALOG)
    {
        ...
        HWND list = GetDlgItem(dialog, IDC_ISSUES_LIST); RECT units = {0, 0, 84, 20};
        MapDialogRect(dialog, &units);
        ListView_SetExtendedListViewStyle(list, LVS_EX_FULLROWSELECT | LVS_EX_DOUBLEBUFFER);
        const char *names[] = {"Severity", "Item", "Issue"};
        for (int i = 0; i < 3; i++)
        {
            LVCOLUMN column = {0}; column.mask = LVCF_TEXT | LVCF_WIDTH;
            column.pszText = (char *)names[i]; column.cx = i == 0 ? units.right : units.right * 2;
    ...
    {
        RECT minimum = {0, 0, 480, 270}; MapDialogRect(dialog, &minimum);
...
BOOL IssuesWindowShow(HWND owner, const char *level, const char **why)
{
    *why = "Could not open Check for Issues.";
    ...
    char title[128]; snprintf(title, sizeof(title), "Check for Issues - %s", level);
    SetWindowText(g_IssuesWindow, title); ShowWindow(g_IssuesWindow, SW_RESTORE); SetForegroundWindow(g_IssuesWindow);
