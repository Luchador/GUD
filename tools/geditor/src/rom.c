/**
 * GUD ROM validation checks. Different failure modes provide specific feedback to the user.
 */
...
#define ROM_MIN_SIZE (1024u * 1024u)        /* Smaller than 1 MB is almost certainly not a ROM */
#define ROM_MAX_SIZE (64u * 1024u * 1024u * 2u)  /* Twice larger than the biggest N64 cart for the ambitious modders out there. */
...
/**
 * The ROM is big-endian, but our PC is not. All multiple byte reads must go through this.
 */
...
/**
 * Reads the whole file and returns NULL on any failure.
 */
...
/**
 * Offset of the manifest magic, or 0 if absent. Struct data is at least 4-aligned, so the scan strides 4.
 */
...
BOOL RomValidate(const char *path, RomInfo info, const char **reasonout)
{
    ...
    *reasonout = "";
    ...
    if (info->entrycount < 1 || info->entrycount > ROM_MAX_ENTRIES || m + 24 + info->entrycount * 16 > size)
    {
        free(data);
