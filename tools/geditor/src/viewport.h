 ...
 * state and S/T wrapping; NULL uses primary/secondary defaults. NULL/0 restores the spinning startup
 * model (an empty canvas for orbit previews). Set framecamera when opening a level; clear it when rebuilding an
...
/* Counts all loaded BG, props and characters (including visible held items),
 * regardless of viewport visibility toggles. Cached when geometry changes. */
