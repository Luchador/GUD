...
Rebuild **both GUD and GEditor**. Current projects use `.gep` version 5 and
a base ROM with 44-byte `STGT` rows. Earlier projects are unsupported; create
a fresh project from the rebuilt ROM. The memory controls are disabled when
the project's base ROM does not expose `LMEM` slots.

GUD retains the existing allocation tokens and default values, but stores each
stage's string in its own reserved 64-byte slot. `STGT` uses its 44-byte layout
...
version 5. All projects use that version, including ones without memory
overrides. Rebase merges each budget separately:
