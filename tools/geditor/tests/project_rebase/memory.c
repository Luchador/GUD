static void MemoryRebases(const GEditorProject *source,const char *incoming,const char *parent)
{
    ...
    OK(ProjectRebaseCreate(source,path,FALSE,parent,"MemorySupported",&supported,&report,&why));
    ...
    OK(ProjectRebaseCreate(&supported,path,FALSE,parent,"MemoryUpdated",&updated,&report,&why));
    ...
    OK(ProjectRebaseCreate(&updated,path,FALSE,parent,"MemoryAgain",&again,&report,&why));
    ...
    OK(!ProjectRebaseCheck(&supported,path,FALSE,&report,&why)&&report.conflicts&&strstr(report.details,"Display lists"));
    ...
    OK(ProjectRebaseCreate(&supported,path,FALSE,parent,"MemoryMatched",&again,&report,&why)&&!again.memoryOverrides.count);
