#!/usr/bin/env python3
"""BG midpoint subdivision, persistence, history and Ctrl+Q routing.
Stan subdivision coverage lives in stan_topology/run.py.
"""
...
with tempfile.TemporaryDirectory(prefix='geditor-bisect-') as temp:
    work = Path(temp)
    ...
    (work / 'editor.inc').write_text(''.join(helpers.function(editor, n) for n in
        ('GEditorBisectSelectedEdge', 'GEditorHandleBisectEdgeHotkey')))
