...
static void Reject(SetupFile *setup)
{
    ...
    DWORD count = 0;
    const char *why;
    assert(!SetupFileBuildPatrolLinks(setup, &links, &count, &why));
...
static void Synthetic(void)
{
    ...
    assert(SetupFileBuildPatrolLinks(&setup, &links, &count, &why) && count == 3);
    ...
    data[125] = 0;
    assert(SetupFileBuildPatrolLinks(&setup, &links, &count, &why) && count == 2);
    ...
    assert(SetupFileBuildPatrolLinks(&setup, &links, &count, &why) && !count); free(links);
    ...
    Put(data + 16, 0);
    assert(SetupFileBuildPatrolLinks(&setup, &links, &count, &why) && !count && !links);
...
int main(int argc, char **argv)
{
    ...
    for (int i = 2; i < argc; i++)
    {
        ...
        if (!SetupFileBuildPatrolLinks(&setup, &links, &count, &why))
        { fprintf(stderr, "%s: %s\n", argv[i], why); abort(); }
