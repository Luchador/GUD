...
static void RomAndProject(const char *dir, DWORD rowSize)
{
    ...
    Put32(row,9);Float(row+20+shift,1.25f);Float(row+24+shift,0.5f);row[29+shift]=13;row[31+shift]=14;row[33+shift]=15;
    for(i=0;i<5;i++)
    {
    ...
    assert(Get32(rom.data+LEVELS+20+shift)==0x40000000&&Get32(rom.data+LEVELS+24+shift)==0x3e800000);
    assert(rom.data[LEVELS+29+shift]==21&&rom.data[LEVELS+31+shift]==22&&rom.data[LEVELS+33+shift]==23);
    assert(!memcmp(rom.data+LEVELS,data+LEVELS,20+shift));
...
int main(int argc,char **argv) { assert(argc==2);RomAndProject(argv[1],36);RomAndProject(argv[1],40);VertexBatches();return 0; }
