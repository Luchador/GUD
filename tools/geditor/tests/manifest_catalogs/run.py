...
def main():
    ...
    assert blob[:16] == b"GUDGEDITORMANIF\0" and version == 3 and count == 33
    ...
    assert set(entries) == set("IMGS OBSG MUSF STGT CMAP FTBL ENVT TXTB TXCF MONA MONT MOND "
                               "SFXC SFXT INSC INST MUST MUSD MUSV ANID ANIF ANIC ANIO CHRM PROP ITEM AIGL TXBK NPRP NPMD LMEM OCCL OFAD".split())
    assert struct.unpack_from(">III", blob, entries["OCCL"] + 4) == (0, 0, 1)
    assert struct.unpack_from(">III", blob, entries["OFAD"] + 4) == (0, 0, 1)
    ...
    print("PASS: 33 manifest kinds, catalog ABI/counts/terminators, native pointer relocations and raw segment anchors.")
