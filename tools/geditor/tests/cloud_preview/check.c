...
static void Environment(unsigned char *p, DWORD id, BOOL enabled, float red)
{
    Put32(p,id); p[44]=16; p[45]=48; p[46]=96; p[47]=enabled;
    Float(p+48,5000); Float(p+56,red); Float(p+60,199); Float(p+64,186); Float(p+92,25);
}
static void Metadata(void)
{
    ...
    rom.info.entries[0]=(RomManifestEntry){0x454e5654,0,0,104};
    rom.info.entries[1]=(RomManifestEntry){0x434d4150,0,sizeof(data),0};
    Environment(data,0xffffffffu,TRUE,10);
    Environment(data+104,209,TRUE,20);
    Environment(data+208,9,TRUE,30);
    Environment(data+312,21,FALSE,40);
    ...
    data[104+53]=3;
    assert(!RomGetLevelClouds(&rom,9,&clouds) && !clouds.enabled);
    data[104+53]=0; Float(data+104+48,NAN);
    assert(!RomGetLevelClouds(&rom,9,&clouds) && !clouds.enabled);
    Float(data+104+48,5000); Float(data+104+56,256);
    assert(!RomGetLevelClouds(&rom,9,&clouds) && !clouds.enabled);
    Float(data+104+56,20); Float(data+104+92,INFINITY);
    assert(!RomGetLevelClouds(&rom,9,&clouds) && !clouds.enabled);
    Float(data+104+92,25);
    rom.info.entries[1].romend=sizeof(data)-1;
    assert(!RomGetLevelClouds(&rom,9,&clouds) && !clouds.enabled); /* Truncated terminator. */
    rom.info.entries[1].romend=sizeof(data); Put32(data+416,99);
    assert(!RomGetLevelClouds(&rom,9,&clouds) && !clouds.enabled);
    Put32(data+416,0); rom.info.entries[0].flags=100;
    assert(!RomGetLevelClouds(&rom,9,&clouds) && !clouds.enabled);
...
int main(void) { Metadata(); ProjectionAndColor(); return 0; }
