...
{ assert(menu && (id==1 || id==2 || id==3));menuitems|=1u<<id;snprintf(id==1?label:id==2?linklabel:seamlabel,sizeof(label),"%s",text);return TRUE; }
...
{ commands++;sent=msg;if(msg==VIEWPORT_WM_SPLIT_EDGE || msg==VIEWPORT_WM_MARK_SEAM) { sentedge=*(const BgDocumentEdgeRef *)lparam;seammark=wparam; }else if(msg==VIEWPORT_WM_SPLIT_STAN_EDGE || msg==VIEWPORT_WM_LINK_STAN_EDGE)sentstanedge=*(const StanEdgeRef *)lparam;else assert(!lparam); }
