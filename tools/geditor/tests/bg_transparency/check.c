...
static void Inspector(void)
{
    ...
    char text[256];
    WriteCommand(commands,0xBA001402,0x00100000);
    ...
    assert(FacePropertiesRenderText(&document,refs,2,text,sizeof(text),NULL));
    assert(!strcmp(text,"Transparency: Auto (Opaque)"));
    assert(FacePropertiesRenderText(&document,refs,3,text,sizeof(text),NULL));
    assert(!strcmp(text,"Transparency: Auto (Mixed)"));
    FacePropertiesRenderText(&document,refs+2,1,text,sizeof(text),NULL);
    assert(strstr(text,"Transparency: Auto (Translucent (alpha blend))"));
    assert(!FacePropertiesRenderText(&document,refs+3,1,text,sizeof(text),NULL));
    assert(strstr(text,"Transparency: Auto (Cutout + blend)"));
    assert(!FacePropertiesRenderText(&document,refs+4,1,text,sizeof(text),NULL));
    assert(strstr(text,"Transparency: Auto (Inherited / unknown)"));
    FacePropertiesRenderText(&document,refs+5,1,text,sizeof(text),NULL);
    assert(strstr(text,"Transparency: Auto (Cutout)"));
    ...
    /* Inspector distinguishes the authoring policy from the native mode. */
    int choice=-1;
    assert(FacePropertiesRenderText(&document,refs,2,text,sizeof(text),&choice) && choice==1);
    unsigned char override[8];
    WriteCommand(override,BG_SURFACE_MARKER,BG_SURFACE_TAG_VALUE(BG_SURFACE_BLEND,0x49d8));
    groups[1]=(BgDocumentDrawGroup){.commands=override,.commandsize=8};
    assert(FacePropertiesRenderText(&document,refs+1,1,text,sizeof(text),&choice) && choice==4);
    assert(!strcmp(text,"Transparency: Translucent (alpha blend)"));
    assert(FacePropertiesRenderText(&document,refs,2,text,sizeof(text),&choice) && choice==0);
...
{
    Decoder(); Inspector();
    puts("PASS BG transparency: native/partial/inherited state, mixed selection, room/layer isolation and conditional editing (ASan + UBSan).");
    return 0;
}
