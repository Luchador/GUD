...
static void Inspector(void)
{
    ...
    assert(FacePropertiesRenderText(&document,refs,2,text,sizeof(text)));
    assert(!strcmp(text,"Transparency: Opaque"));
    assert(FacePropertiesRenderText(&document,refs,3,text,sizeof(text)));
    ...
    assert(!FacePropertiesRenderText(&document,refs+3,1,text,sizeof(text)));
    assert(strstr(text,"Transparency: Cutout + blend"));
    assert(!FacePropertiesRenderText(&document,refs+4,1,text,sizeof(text)));
