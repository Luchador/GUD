...
def function(source, name):
    match = re.search(r'^(?:static )?(?:BOOL|DWORD|void|double|f32|unsigned int) '+name+r'\([^;]*?\)\s*\{', source, re.M)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='geditor-portals-') as temp:
        ...
        prelude = '''#include <assert.h>\n#include <float.h>\n#include <math.h>\n#include <stdio.h>\n#include "bgload.h"
            ...
            typedef struct { int tool; unsigned char portalselection[BG_MAX_PORTALS]; BOOL showportals; BgPortalFile portals; DWORD selectedportal; Vertex *portalfill,*portaledges; } ViewportState;
        '''
        prelude += '\n'.join(line for line in viewport.splitlines() if line.startswith(('#define VIEWPORT_PICK_', '#define VIEWPORT_SELECTION_GOLD')))+'\n'
        prelude += '#define EDITOR_TOOL_FACE_SELECT 2\n'
        for name in ['ViewportPortalComponentMask', 'ViewportRayTriangleDistance', 'ViewportCoplanarPickTolerance', 'ViewportFindPickedPortal', 'ViewportPortalGeometryIsFirst', 'ViewportRefreshPortalColors']:
        ...
        int main(void) {
            ...
            ViewportState state={.tool=EDITOR_TOOL_FACE_SELECT,.showportals=TRUE,.portals={portals,3},.selectedportal=BG_PORTAL_INDEX_NONE,.portalfill=fill,.portaledges=edges};
            ...
            state.selectedportal=1;state.portalselection[1]=1;ViewportRefreshPortalColors(&state);
            ...
            state.selectedportal=BG_PORTAL_INDEX_NONE;state.portalselection[1]=0;ViewportRefreshPortalColors(&state);
