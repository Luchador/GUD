...
int main(void)
{
    ...
    for (int kind = BROWSER_OBJECT_INTRO_CAMERA; kind <= BROWSER_OBJECT_GLASS; kind++)
    {
        Reset(TRUE); Start(TRUE); g_state.pressedobject = kind;
        Dispatch(browser, WM_LBUTTONUP, 0, (100 << 16) | 320);
        assert(objectdrops == 1 && placed.type == kind && placed.screen.x == 420 && placed.screen.y == 300);
    ...
    for (int kind = BROWSER_OBJECT_CCTV; kind <= BROWSER_OBJECT_DRONE_GUN; kind++)
    {
        Reset(TRUE); Start(TRUE); g_state.pressedobject = kind;
        Dispatch(browser, WM_LBUTTONUP, 0, (100 << 16) | 320);
        assert(objectdrops == 1 && placed.type == kind && placed.screen.x == 420 && placed.screen.y == 300);
    ...
    puts("PASS: palette capture, triangle/quad/circle/cylinder/spawn/intro/outro/door/glass/CCTV/alarm/drone/armor drop type/position, capture loss, cancellation, image/model drags.");
    return 0;
}
'''
    dispatch = '\nstatic LRESULT Dispatch(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam)\n{\n'
