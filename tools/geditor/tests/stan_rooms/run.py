#!/usr/bin/env python3
"""Check stan room reassignment, grouped native records, links, persistence and history."""
...
with tempfile.TemporaryDirectory(prefix='geditor-stan-rooms-') as temp:
    work = Path(temp)
    (work / 'stan').mkdir()
    ...
    (work / 'controller.inc').write_text(extract.function((src / 'geditor.c').read_text(), 'GEditorSetSelectedStanRoom'))
    ...
    (work / 'input.inc').write_text(extract.function((src / 'rightpanel.c').read_text(), 'RightPanelParseStanRoom'))
    binary = work / 'check'
    subprocess.run([os.environ.get('CC', 'cc'), '-std=c99', '-O1', '-g', '-Wall', '-Wextra', '-Werror',
                    ...
                    *[str(src / name) for name in ('stanload.c', 'stanedit.c', 'stanquery.c', 'bghistory.c')],
